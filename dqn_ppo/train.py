...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.env = default_registry[cfg.env].make(seed=self.seed, worker_id=cfg.work_id, side_channels=[channel], no_graphics=False)
        self.env.reset()
        self.behavior_name = list(self.env.behavior_specs)[0]

        eval_channel = EngineConfigurationChannel()
        eval_channel.set_configuration_parameters(time_scale=0.0)
        self.eval_env = default_registry[cfg.env].make(seed=self.seed+1, worker_id=cfg.work_id_ev, side_channels=[eval_channel], no_graphics=False)
        self.eval_env.reset()
